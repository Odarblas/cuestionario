document.addEventListener('DOMContentLoaded', function() {
    ...
    validSections.forEach((section, validSectionIndex) => {
            ...
            section.forEach((question, questionIndex) => {
                const globalQuestionIndex = questionCounter++;
    ...
    questionElement.appendChild(container);
        ...
        else if (question.type === "js-class-completion") {
            const container = document.createElement('div');
            container.className = 'js-completion-container';
            ...
            const codeContainer = document.createElement('div');
            codeContainer.className = 'code-completion';
            ...
            lines.forEach(line => {
                ...
                parts.forEach(part => {
                    if (part.match(/\[\d+\]/)) {
                        ...
                        const optionGroup = question.options.find(opt => opt.placeholder === `[\${placeholder}]`);
                        ...
                        if (userAnswers[globalQuestionIndex][placeholder] !== undefined) {
                            select.value = userAnswers[globalQuestionIndex][placeholder];
                        }
   ...
   // Validación de límites
    ...
    function calculateScore() {
        ...
        userAnswers.forEach((answer, index) => {
            ...
            // Dentro de calculateScore()
else if (question.type === "css-filter-dropdown") {
    let correctCount = 0;
    
    question.examples.forEach((example, index) => {
        if (userAnswers[globalQuestionIndex] && 
            userAnswers[globalQuestionIndex][index] === example.correctFilter) {
            correctCount++;
        }
    });
    
    // Dar puntos parciales por cada respuesta correcta
    score += correctCount * (1 / question.examples.length);
}
            else if (question.type === "matching") {
                let allCorrect = true;
                question.matchingQuestion.items.forEach(item => {
                    if (answer[item.id] == undefined || answer[item.id] != item.correctRight) {
                        allCorrect = false;
                    }
                });
                if (allCorrect) score++;
    ...
    question.statements.forEach((statement, index) => {
        const userAnswer = answer ? answer[index] : null;
    ...
    function showResults() {
        ...
        allQuestions.forEach((question, index) => {
            ...
            answerElement.innerHTML = `<h4>Pregunta ${index + 1}: ${question.question}</h4>`;
            
            if (question.type === "dynamic-selector") {
